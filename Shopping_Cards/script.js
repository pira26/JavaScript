...
addToCartButtonsDOM.forEach(addToCartButtonDOM => {
    addToCartButtonDOM.addEventListener('click', () => {
        ...
        const isInCart = (carts.filter(cart => (cart.name === product.name)).length > 0);
...
function increaseItem(product, cartItemDOM) {
    carts.forEach((cart) => {
        if (cart.name === product.name) {
            cartItemDOM.querySelector('.cart-item-quantity').innerText = ++cart.quantity;
...
function removeItem(product, cartItemDOM, addToCartButtonDOM) {
    ...
    carts = carts.filter(cartItem => cartItem.name !== product.name);
    saveCart();
...
function checkout() {
    let paypalFormHTML = `
    <form id="paypal-form" action="https://www.paypal.com/fr" method="post">
  ...
  `;
    ...
    carts.forEach((cart, index) => {
        ++index;
        paypalFormHTML += `
      <input type="hidden" name="item-name_${index}" value="${cart.name}">
      <input type="hidden" name="amount_${index}" value="${cart.price}">
...
function countCartTotal() {
    let cartTotal = 0;
    carts.forEach((cart) => cartTotal += cart.quantity * cart.price);
